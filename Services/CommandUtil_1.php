...
class CommandUtil
{
    ...
    public function __construct(Kernel $kernel, CommandConfiguration $commandConfiguration, $max_execution_time, $execution_mode)
    {
        $this->kernel               = $kernel;
        $this->loopArgValues              = array();
    ...
    public function run($definition, $values)
    {
        ...
        if(!empty($this->loopArgValues))
        {
            $sep = '-###-';
            foreach($this->loopArgValues as $value)
            {
                $cmd = str_replace('%loop_arg%', $value, implode($sep, $params));
                $execParams = explode($sep, $cmd);
                
                $content .= $this->execute($execParams).PHP_EOL;
            }
        }
        else
        {
    ...
    protected function formatExecParams($type, $definition, $values, &$params)
    {
        foreach($definition[$type] as $name => $def)
        {
            if(isset($values[$name]) && ($value = $values[$name]))
            {
                ...
                {
                    //if an argument as array of values, it is necessarely on loopable behaviour
                    if(is_array($value))
                    {
                        $params[] = '%loop_arg%';
                        $this->loopArgValues = $value;
                    }
                    else
                    {
                        $params[] = trim($value);
